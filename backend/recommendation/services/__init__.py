...
# from .recommendation import AnalyticsService
...
class ReviewService:
    ...
    def create_review(self, user: TUser, payload: CreateReviewSchema):
        ...
        order = self.order_service.get_order_by_uid(payload.order_uid)
        ...
        if not self.order_service.check_dish_in_order(order, dish):
            raise DishNotOrderedException
    ...
    def get_reviews_by_dish(self, dish_uid: UUID):
        """Lấy tất cả reviews của một dish"""
        # Check dish exists (using DishService)
        dish = self.dish_service.get_dish_by_uid(dish_uid)
        if not dish:
            raise DishNotFoundException
        
        return self.orm.get_reviews_by_dish(dish_uid)
